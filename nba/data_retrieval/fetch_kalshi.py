...
KALSHI_PRIVATE_KEY_PATH = os.getenv(
    ...
    str(PROJECT_ROOT / "gui" / "python_app" / "kalshi_client" / "kalshi_key.pem")
)
...
def generate_kalshi_tickers(away_team: str, home_team: str, game_date: pd.Timestamp) -> Tuple[str, str]:
    """
    ...
    home_code = TEAM_CODES[home_team]
    away_code = TEAM_CODES[away_team]
...
def get_game_window(pbp_df: pd.DataFrame) -> Tuple[pd.Timestamp, pd.Timestamp]:
    """
    Uses first/last action times in pbp_df['timeActual'].
    Returns UTC timestamps (no buffer - exact game window).
    """
    if "timeActual" not in pbp_df.columns:
        raise ValueError("PBP CSV must contain a 'timeActual' column.")

    # Force UTC parsing
    pbp_df = pbp_df.copy()
    pbp_df["timeActual"] = pd.to_datetime(pbp_df["timeActual"], utc=True, errors="coerce")
    pbp_df = pbp_df.dropna(subset=["timeActual"]).sort_values("timeActual")

    if pbp_df.empty:
        raise ValueError("No valid timestamps found in 'timeActual'.")

    game_start_utc = pbp_df["timeActual"].min()
    game_end_utc = pbp_df["timeActual"].max()
...
def _get_yes_price_cents(trades_df: pd.DataFrame) -> pd.Series:
    ...
    raise ValueError("Could not find a YES price column in trades (expected 'price' or 'yes_price').")


def _get_trade_count(trades_df: pd.DataFrame) -> pd.Series:
    """Get trade size/count from trades"""
    for col in ("count", "size", "volume", "qty"):
        if col in trades_df.columns:
...
def read_first_game_from_schedule(schedule_csv: str) -> Tuple[pd.Timestamp, str, str]:
    """
    Returns (game_date, away_team_name, home_team_name) for the FIRST row.
    Handles both explicit HOME/AWAY columns and MATCHUP column format.
    """
    sched = pd.read_csv(schedule_csv)

    date_col = _first_existing_col(sched, ["GAME_DATE", "game_date", "DATE", "Date"])
    first = sched.iloc[0]
    game_date = pd.to_datetime(first[date_col])

    # Check if we have a MATCHUP column (nba_api format)
    if "MATCHUP" in sched.columns:
        matchup = str(first["MATCHUP"])
        away_code, home_code = _parse_matchup_to_teams(matchup)
        away_team = _code_to_team_name(away_code)
        home_team = _code_to_team_name(home_code)
    else:
        # Fallback to explicit columns
        home_col = _first_existing_col(sched, ["HOME_TEAM_NAME", "HOME_TEAM", "home_team", "Home Team", "HOME"])
        away_col = _first_existing_col(sched, ["VISITOR_TEAM_NAME", "AWAY_TEAM_NAME", "AWAY_TEAM", "away_team", "Away Team", "VISITOR", "AWAY"])
        home_team = str(first[home_col])
        away_team = str(first[away_col])

    return game_date, away_team, home_team


def read_all_games_from_schedule(schedule_csv: str) -> List[Tuple[pd.Timestamp, str, str]]:
    """
    Returns list of (game_date, away_team_name, home_team_name) for ALL rows.
    Handles both explicit HOME/AWAY columns and MATCHUP column format.
    """
    sched = pd.read_csv(schedule_csv)
    date_col = _first_existing_col(sched, ["GAME_DATE", "game_date", "DATE", "Date"])
    ...
    for idx, row in sched.iterrows():
        ...
        games.append((game_date, away_team, home_team))
    
    return games


def parse_pbp_filename(pbp_file: str) -> Tuple[pd.Timestamp, str, str]:
    """
    Parse game info from PBP filename.
    Expected format: YYYYMMDD_AWAY_HOME.csv
    Returns: (game_date, away_team_code, home_team_code)
    """
    base = os.path.basename(pbp_file).replace(".csv", "")
    parts = base.split("_")
    if len(parts) < 3:
        raise ValueError(f"Invalid PBP filename format: {pbp_file}. Expected: YYYYMMDD_AWAY_HOME.csv")
...
def get_output_path(pbp_file: str, out_dir: Optional[str] = None) -> str:
    ...
    if not out_dir:
        out_dir = "nba/data/kalshi_live"
    base = os.path.basename(pbp_file).replace(".csv", "_kalshi_100ms.csv")
    return os.path.join(out_dir, base)


def find_pbp_file(pbp_dir: str, game_date: pd.Timestamp, away_team: str, home_team: str) -> Optional[str]:
    """
    Best-effort heuristic: looks for files containing:
      - YYYYMMDD
      - both team codes (e.g., DEN and MEM)
    """
    ...
    patterns = [
        os.path.join(pbp_dir, f"*{ymd}*{home_code}*{away_code}*.csv"),
        os.path.join(pbp_dir, f"*{ymd}*{away_code}*{home_code}*.csv"),
        os.path.join(pbp_dir, f"*{ymd}*.csv"),
    ]
    for pat in patterns:
...
def process_one_game(
    ...
    home_ticker, away_ticker = generate_kalshi_tickers(away_team, home_team, game_date)
    ...
    combined.insert(1, "home_team", home_team)
    combined.insert(2, "away_team", away_team)
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog="""
Usage modes:
  1. Process all PBP files in a directory: --pbp_dir path/to/games_live/
  2. Process from schedule CSV: --schedule_csv path.csv --pbp_dir path/to/games_live/
  3. Process single game: --pbp_file path.csv --game_date YYYY-MM-DD --home_team "Team Name" --away_team "Team Name"
        """
    )

    # Batch mode: process all games from schedule
    ap.add_argument("--schedule_csv", help="Schedule CSV; processes ALL games (use --first-only to process just the first).")
    ...
    ap.add_argument("--first-only", action="store_true", help="Only process the first game in the schedule CSV.")

    # Single-game mode (explicit pbp file + metadata)
    ap.add_argument("--pbp_file", help="Explicit PBP CSV file to process.")
    ap.add_argument("--game_date", help="Game date YYYY-MM-DD (required with --pbp_file unless embedded in schedule)")
    ap.add_argument("--home_team", help='Home team full name (e.g., "Denver Nuggets")')
    ap.add_argument("--away_team", help='Away team full name (e.g., "Memphis Grizzlies")')

    ap.add_argument("--out_dir", default=None, help="Where to write output CSV (default: nba/data/kalshi_live).")
    ...
    if args.schedule_csv:
        ...
        if args.first_only:
            # Only process the first game
            game_date, away_team, home_team = read_first_game_from_schedule(args.schedule_csv)
            pbp_file = find_pbp_file(args.pbp_dir, game_date, away_team, home_team)
            if not pbp_file:
                raise SystemExit(
                    f"Could not locate a PBP CSV in {args.pbp_dir} for first game: "
                    f"{game_date.date()} {away_team} @ {home_team}. "
                    f"Adjust find_pbp_file() to match your filename scheme."
                )

            out = process_one_game(pbp_file, client, game_date, away_team, home_team, out_dir=args.out_dir)
            print(f"[OK] Wrote: {out}")
            return

        # Process ALL games
        games = read_all_games_from_schedule(args.schedule_csv)
        print(f"Found {len(games)} games in schedule CSV")
        ...
        for i, (game_date, away_team, home_team) in enumerate(games, 1):
            print(f"\n[{i}/{len(games)}] {game_date.date()}: {away_team} @ {home_team}")
            
            # Find PBP file
            pbp_file = find_pbp_file(args.pbp_dir, game_date, away_team, home_team)
            if not pbp_file:
                print(f"  SKIP: No PBP file found")
                no_pbp_count += 1
                continue
    ...
    if args.pbp_dir and not args.schedule_csv:
        ...
        for pbp_file in pbp_files:
            try:
                ...
                game_date, away_code, home_code = parse_pbp_filename(pbp_file)
                away_team = team_code_to_full_name(away_code)
                home_team = team_code_to_full_name(home_code)
                
                print(f"\n[{processed + 1}/{len(pbp_files)}] Processing: {os.path.basename(pbp_file)}")
                print(f"  Game: {away_team} @ {home_team} on {game_date.strftime('%Y-%m-%d')}")
    ...
    # --- SINGLE GAME MODE ---
    if args.pbp_file:
        if not (args.game_date and args.home_team and args.away_team):
            raise SystemExit("For --pbp_file mode, you must also pass --game_date, --home_team, --away_team.")

        game_date = pd.to_datetime(args.game_date)
        out = process_one_game(args.pbp_file, client, game_date, args.away_team, args.home_team, out_dir=args.out_dir)
        print(f"[OK] Wrote: {out}")
        return
