...
async def select_game(request: Request):
    try:
        return await _select_game(request)
    except Exception as e:
        import traceback, sys
        print("[select_game] CRASHED:", file=sys.stderr)
        traceback.print_exc()
        return {"success": False, "error": f"{type(e).__name__}: {e}"}

async def _select_game(request: Request):
    body = await request.json()
    ...
    replay_loaded = False
    if game.get("orderbook_file") and replay_path.exists():
        game_meta = {
    ...
    if game.get("pbp_file") and pbp_path.exists():
    ...
    if snapshot_feeder and game.get("nba_pbp_json") and nba_pbp_path.exists():
        ...
        # Auto-align: shift PBP timestamps so the first action lines up with
        # the first orderbook event. Allows orderbook captures from any later
        # session to be replayed alongside the actual game PBP without manual
        # offset tuning. Can be overridden by setting "pbp_offset_sec" in games.json.
        if "pbp_offset_sec" in game:
            replay_engine.pbp_offset_sec = float(game["pbp_offset_sec"])
        elif replay_engine.events and snapshot_feeder.actions:
            ob_first_sec = replay_engine.events[0].wall_ms / 1000.0
            pbp_first_sec = snapshot_feeder.actions[0].ts
            replay_engine.pbp_offset_sec = ob_first_sec - pbp_first_sec
            print(f"[select_game] auto pbp_offset_sec = {replay_engine.pbp_offset_sec:.0f}s "
                  f"(orderbook starts {(replay_engine.pbp_offset_sec/3600):.1f}h after PBP)")

    # Clear any cached candlesticks from the previous game
    try:
        from . import rest_api as _rest_api
        _rest_api._candles_cache.clear()
    except Exception:
        pass
